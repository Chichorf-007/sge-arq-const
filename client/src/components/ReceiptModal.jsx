...
export default function ReceiptModal({ settlement, token, onClose, onPaymentRegistered, alreadyPaid }) {
  const [isPaid, setIsPaid] = useState(!!alreadyPaid);
  ...
  const isHistorical = period_start === 'Histórico' || period_end === 'Histórico';
  ...
  const handleMarkAsPaid = async () => {
    ...
    try {
      ...
      // CRÍTICO: si falla, NO marcar isPaid como true (evitar falso positivo de "pagado")
      setRegisterError(err.message || 'Error al registrar el pago');
  ...
  const renderReceiptBody = (copyLabel) => (
    <div style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", padding: '1rem' }}>
      {/* Copy Label */}
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      ...
      >
        ...
        <div className="no-print" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', borderBottom: '1px solid #eee', paddingBottom: '1rem', flexWrap: 'wrap', gap: '0.75rem' }}>
            ...
            {!isHistorical && (
              isPaid ? (
                <span style={{
                  ...
                  <CheckCircle2 size={16} /> Pago registrado y guardado en el historial
                </span>
              ) : (
                <button onClick={handleMarkAsPaid} className="btn btn-primary btn-sm" disabled={registering}>
                  ✅ {registering ? 'Registrando...' : 'Marcar como Pagado'}
                </button>
              )
            )}
        ...
        {/* PRINTABLE RECEIPT CONTAINER — two copies: proyectista + legajo de Silvia */}
        <div id="printable-receipt">
          <div className="receipt-page">
            {renderReceiptBody('COPIA PARA: PROYECTISTA')}
          </div>

          <hr style={{ margin: '2rem 0', border: 'none', borderTop: '2px dashed #CBD5E1' }} className="no-print" />

          <div className="receipt-page">
            {renderReceiptBody('COPIA PARA: SILVIA GONZÁLEZ (LEGAJO)')}
          </div>
        </div>
      </div>
    </div>
